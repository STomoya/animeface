...
'''
BigGAN with Squeeze and Excitation as Non-Local layer
'''
...
class SENet(nn.Module):
    def __init__(self,
        ...
        self.squeeze_ = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Flatten(),
            Linear(channels, channels//self.ratio, use_sn, sn_eps, bias=False),
            get_activation('relu', inplace=True),
            Linear(channels//self.ratio, channels, use_sn, sn_eps, bias=False),
            get_activation('sigmoid')
        )
    
    def forward(self, x, y=None):
        heat = self.squeeze_(x)
        heat = heat.unsqueeze(-1).unsqueeze(-1)
        return x * heat

def NonLocal(channels, use_sn, sn_eps=1e-12, name='sa', resl=64):
    if name == 'sa':
        return SelfAttention(channels, use_sn, sn_eps)
    elif name == 'll':
        return LambdaLayer(channels, use_sn, sn_eps, resl**2)
    elif name == 'se':
        return SENet(channels, use_sn, sn_eps)
...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        att_name='se', act_name='relu', norm_name='bn',
        mlp=False
    ):
        super().__init__()
        ...
        for index, resl in enumerate(arch['resl']):
            ...
            if resl == arch['att'] and att:
                att = False
                self.blocks.append(
                    NonLocal(arch['out'][index]*channels, use_sn, sn_eps, att_name, resl)
                )
...
class Discriminator(nn.Module):
    ...
    '''
    def __init__(self,
        image_size, deep, channels=64, use_sn=True, att_name='se', act_name='relu'
    ):
        super().__init__()
