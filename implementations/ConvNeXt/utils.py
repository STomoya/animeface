    ...
    while not status.is_end():
        for real in dataset:
            ...
            if status.batches_done % save == 0:
                ...
                save_image(images, f'implementations/ConvNeXt/result/{status.batches_done}.jpg',
                    nrow=4, normalize=True, value_range=(-1, 1))
                torch.save(G.state_dict(), f'implementations/ConvNeXt/result/model{status.batches_done}.pth')
...
def main(parser):
    ...
    amp = not args.disable_amp and args.disable_gpu
    ...
    G = Generator(
        ...
        args.max_channels, args.block_type, args.blocks_per_scale)
    G.apply(initobj.N002)
    G_ema = copy.deepcopy(G)
    freeze(G_ema)
    D = Discriminator(args.image_channels)
    D.apply(initobj.N002)
